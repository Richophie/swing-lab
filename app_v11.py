...
def latest_v11():
    ...
    data['results'] = [r for r in (data.get('results') or []) if r.get('grade') in {'S', 'A'} and r.get('eligible', True)]
    data['version'] = '11.0'
...
def index_v11():
    ...
    html = html.replace('PRO LIVE v10.2', 'PRO LIVE v11.0').replace('MULTI-STRATEGY ROUTER', 'DAILY SIGNAL JOURNAL')
...
function historyCard(x){let p={entry_low:x.entry_low,entry_high:x.entry_high,target:x.target,stop:x.stop},r={sparkline:x.sparkline||[],trade_plan:p};return `<div class="pick"><div class="picktop"><div><div class="ticker">${x.symbol}</div><div class="mut">${x.strategy_name||''}</div></div><div style="text-align:right"><div class="grade ${x.grade==='S'?'good':'warn'}">${x.grade}</div><b class="${outcomeClass(x)}">${outcomeText(x)}</b></div></div><div class="chartmini">${mini(r)}</div><div class="levels"><div>BUY<b>$${x.entry_low??'-'}~${x.entry_high??'-'}</b></div><div>TARGET<b>$${x.target??'-'}</b></div><div>STOP<b>$${x.stop??'-'}</b></div></div><div class="metrics"><span>RSI ${x.rsi??'-'}</span><span>120일선 ${x.d120??'-'}%</span><span>볼린저 ${x.bb_pos??'-'}%</span><span>목표 ${x.target_days_low??'-'}~${x.target_days_high??'-'}일</span></div><div class="result" style="margin-top:8px;padding:9px 10px"><b>추천 이유 · ${x.strategy_name||''}</b><br><span class="mut">${strategyExplain(x)}</span></div>${x.outcome_note?`<div class="e" style="margin-top:7px">${x.outcome_note}</div>`:''}</div>`;}
...
'''
    ...
    # Strategy copy: show the active signal and what it means, not agreement as a quality score.
    html = html.replace("4개 전략 중 ${en.agreement}개가 현재 매수 신호 · 확신도 ${en.confidence}", "4개 전략 중 ${en.agreement}개가 현재 매수 신호 | ${friendlyEnsemble(en)}")
...
def version_v11():
    return {'version': '11.0', 'features': ['A/S-only public dashboard', 'immutable daily recommendation snapshots', 'infinite date history', 'success/stop/target-miss outcome tracking', 'training-ready realized outcomes']}
