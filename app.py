...
from market_data import load_price_history, indicators, market_snapshot
...
def public_row(raw):
    row=dict(raw);signals=[s for s in row.get('strategy_signals',[]) if s.get('strategy_id') in PUBLIC_STRATEGIES and float(s.get('strategy_score',0))>=85]
    if not signals:return None
    signals.sort(key=lambda x:float(x.get('strategy_score',0)),reverse=True);best=signals[0];plans=row.get('strategy_trade_plans') or {}
    row['strategy_signals']=signals;row['strategy_id']=best['strategy_id'];row['strategy_name']=best['strategy_name'];row['strategy_reason']=best.get('evidence') or best.get('why');row['score']=float(best['strategy_score']);row['trade_plan']=normalize_plan(plans.get(best['strategy_id']) or row.get('trade_plan'));row['name_ko']=row.get('name_ko') or korean_name(row.get('symbol'),row.get('security_name'));return row
...
def chart_payload(symbol,strategy_id,days=180):
    d=load_price_history(symbol,'2y');ind=indicators(d);plan=normalize_plan(trade_plan(d,strategy_id));x=d.join(ind[['sma120','rsi','bb_low','bb_high']],how='left').tail(days);series=[]
...
def version():return jsonify({'version':APP_VERSION,'core':CORE_VERSION,'architecture':'standalone','public_strategies':list(PUBLIC_STRATEGIES)})
...
def latest():
    ...
    rows.sort(key=lambda x:x['score'],reverse=True);data['results']=rows;data['ui_version']=APP_VERSION;data['display_filter']='public S only';return jsonify(data)
...
def detail(symbol):
    try:
        s=symbol.upper().strip();d=load_price_history(s,'10y');state=market_snapshot().get('state');ev=evaluate_strategies(d,state);public=[x for x in ev['strategies'] if x['id'] in PUBLIC_STRATEGIES];public.sort(key=lambda x:(x['active'],x['score']),reverse=True);sid=request.args.get('strategy') or public[0]['id']
        if sid not in PUBLIC_STRATEGIES:sid=public[0]['id']
        chosen=next(x for x in ev['strategies'] if x['id']==sid);plan=normalize_plan(trade_plan(d,sid));official=quote_name(s)
        return jsonify({'symbol':s,'name_ko':korean_name(s,official),'security_name':official,'strategy_id':sid,'strategy_name':chosen['name'],'strategy_reason':chosen['evidence'],'signal':{'score':chosen['score'],'active':chosen['active'],**ev['metrics']},'strategies':[x for x in ev['strategies'] if x['id'] in PUBLIC_STRATEGIES],'trade_plan':plan,'market':market_snapshot()})
