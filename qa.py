...
def check_scan(path=ROOT/'static'/'latest_scan.json'):
    ...
    return {'rows':len(rows),'elite_signals':elite,'candidates':data.get('candidate_count',0),'failed':data.get('failed_count',0),'name_warnings':len(name_warnings)}
...
def check_routes():
    ...
    required={'/','/api/latest','/api/history','/api/detail/<symbol>','/api/chart/<symbol>','/api/backtest/<symbol>','/api/market','/api/version','/health'}
    missing=required-set(routes);assert not missing,missing
    return routes
