...
from config import APP_VERSION, CORE_VERSION, S_THRESHOLD, ELITE_MAX

ROOT=Path(__file__).parent;LEGACY_PREFIXES=('app_v','core_v','scanner_v','trade_journal_v');HANGUL=re.compile(r'[가-힣]')


def check_scan(path=ROOT/'static'/'latest_scan.json'):
    data=json.loads(path.read_text(encoding='utf-8'));assert data.get('version')==APP_VERSION,(data.get('version'),APP_VERSION);assert data.get('core_version')==CORE_VERSION,(data.get('core_version'),CORE_VERSION);rows=data.get('results') or [];symbols=[];elite=0
    for row in rows:
        symbol=row.get('symbol');assert symbol;symbols.append(symbol);name=row.get('name_ko') or '';assert HANGUL.search(name),f'Korean display name missing: {symbol} -> {name!r}';sigs=row.get('strategy_signals') or [];assert sigs,symbol;plans=row.get('strategy_trade_plans') or {}
        for sig in sigs:
...
def check_frontend():
    html=(ROOT/'static'/'dashboard.html').read_text(encoding='utf-8');css=ROOT/'static'/'dashboard.css';js=ROOT/'static'/'dashboard.js';src=js.read_text(encoding='utf-8');assert css.exists() and js.exists(),'dashboard css/js missing';assert '<style' not in html.lower();assert '<script>' not in html.lower();assert '/static/dashboard.css' in html and '/static/dashboard.js' in html;assert 'app_v' not in html and 'PRO LIVE v' not in html;assert 'todayTabs' in html;assert 'rangePrice' in src and 'fxRate' in src and 'target_pct' in src and 'elite_max' in src
    return ['dashboard.html','dashboard.css','dashboard.js']

if __name__=='__main__':print('imports',check_imports());print('routes',check_routes());print('frontend',check_frontend());print('scan',check_scan())
