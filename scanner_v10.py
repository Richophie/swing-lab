...
def grade_for(ens):
    ...
    return 'S' if float(ens.get('ensemble_score', 0)) >= 85 else 'A'
...
def main():
    ...
    promoted=dedupe_share_classes([r for r in base if r.get('grade') in {'S','A'} and r.get('eligible')])
    final=[]
    for r in promoted[:30]:
        try:
            d=history(r['symbol'],'10y')
            r['trade_plan']=trade_plan_for(d,r['strategy_id'])
            r['history_stats']=historical_stats(d)  # legacy context only; not used to rank four strategies
            final.append(r)
        except Exception as exc:
            r['detail_error']=str(exc); final.append(r)
    final.sort(key=lambda x:(1 if x.get('grade')=='S' else 0,x.get('score',0)),reverse=True)
    payload={'status':'ready','version':'11.3','core_version':'4.0','scanned_at':datetime.now(timezone.utc).isoformat(timespec='seconds'),'universe_count':len(universe),'candidate_count':len(candidates),'failed_count':failed,'market':market,'results':final,'ranking_note':'A/S만 공개 · 4개 전략을 동일 품질척도로 정규화 · 전략별 BUY/TARGET/STOP 적용','journal_quality':'experimental_until_strategy_backtests_pass'}
