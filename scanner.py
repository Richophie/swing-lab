        ...
        try:
            ...
            for sig in row['strategy_signals']:
                ...
                if sig.get('experimental'):
                    sig.update({'elite_pass':False,'elite_score':sig['strategy_score'],'research_rank_score':sig['strategy_score'],'selection_reason':'실험 전략 · 엄선에서 제외'});continue
                ...
                research=overlay_for_strategy(sid,assessment['elite_score'])
            ...
            if elite:
                elite.sort(key=lambda s:(float(s.get('research_rank_score') or s['elite_score']),float(s['elite_score'])),reverse=True);best=elite[0];row.update({'strategy_id':best['strategy_id'],'strategy_name':best['strategy_name'],'strategy_reason':best['evidence'],'score':best['elite_score'],'elite_pass':True,'elite_score':best['elite_score'],'research_rank_score':best.get('research_rank_score',best['elite_score']),'research':best.get('research'),'trade_plan':plans[best['strategy_id']]})
            else:
                row['elite_pass']=False;row['trade_plan']=plans.get(row['strategy_id'])
                best_public=sorted([s for s in row['strategy_signals'] if not s.get('experimental')],key=lambda s:(float(s.get('research_rank_score') or s.get('elite_score') or s.get('strategy_score') or 0),float(s.get('elite_score') or s.get('strategy_score') or 0)),reverse=True)
                if best_public:
                    best=best_public[0];row['research_rank_score']=best.get('research_rank_score',best.get('elite_score',best.get('strategy_score')));row['research']=best.get('research')
            out.append(row)
