...
def live_refresh_v10():
    try:
        ...
        return {**base,'version':'10.1','core_version':'3.0','market':market,'results':out,'live_failed':failed,'live_refreshed_at':datetime.now(timezone.utc).isoformat(timespec='seconds'),
...
def index_v10():
    ...
    html=html.replace('오늘의 스윙자리 v8','오늘의 스윙자리 v10').replace('PRO LIVE v8.0','PRO LIVE v10.1').replace('TECH CHART + BACKTEST','MULTI-STRATEGY ROUTER')

    # Dashboard: expose the winning playbook in plain language on every recommendation card.
    html=html.replace(
        "<div style=\"margin-top:8px\"><b>${Math.round(r.score)}점 · ${gt(r.grade)}</b></div>",
        "<div style=\"margin-top:8px\"><b>${Math.round(r.score)}점 · ${r.grade==='S'?'강력 후보':r.grade==='A'?'관찰 후보':'조금 더 기다림'}</b></div><div class=\"result\" style=\"margin-top:8px;padding:9px 10px\"><b>추천 방식 · ${r.strategy_name||'분석 중'}</b><br><span class=\"mut\">${friendlyReason(r)}</span></div>"
    )
...
def version_v10():
    return {'version':'10.1','core':'3.0','architecture':'independent playbooks + regime routing, not indicator soup','playbooks':['confirmed_pullback','rsi2_trend_reversion','momentum_pullback','volatility_breakout'],'ux':['plain-language strategy explanation','formatted KRW inputs','strategy label on dashboard']}
