...
def load_price_history(symbol: str, period: str='10y') -> pd.DataFrame:
    ...
    d=yf.Ticker(s).history(period=period,auto_adjust=False)
...
def market_snapshot() -> dict:
    ...
    for symbol in ('SPY','QQQ'):
        try:
            d=load_price_history(symbol,'2y')
            if len(d)<205: raise ValueError('일봉 부족')
            x=indicators(d).iloc[-1]
            above120=bool(x['close']>x['sma120']); above200=bool(x['close']>x['sma200']); rsi=float(x['rsi'])
            score=int(above120)+int(above200)+int(rsi>45); total+=score
            details[symbol]={'score':score,'rsi':round(rsi,1),'above120':above120,'above200':above200}
        except Exception as exc:
            details[symbol]={'error':str(exc),'score':0};
    ...
    reasons=[]
    for s,q in details.items():
        if 'error' in q: reasons.append(f'{s} 확인 실패')
        else: reasons.append(f"{s} {'120·200일선 위' if q['above120'] and q['above200'] else '장기선 일부 이탈'} · RSI {q['rsi']}")
...
def clean_symbol(sym: str) -> bool:
    return bool(re.match(r'^[A-Z][A-Z0-9.\-]{0,7}$',str(sym or '').strip().upper()))
...
def load_us_universe() -> list[dict]:
    sources=[('https://www.nasdaqtrader.com/dynamic/SymDir/nasdaqlisted.txt','nasdaq'),('https://www.nasdaqtrader.com/dynamic/SymDir/otherlisted.txt','other')]
    rows=[]
