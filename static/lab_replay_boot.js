(()=>{
  ...
  function ensureSection(){
    ...
    s.innerHTML='<article class="panel"><div class="panel-head"><div><div class="eyebrow">REPLAY LAB · 과거 시뮬레이션</div><h2>백테스트연구소</h2><p class="paper-head-note">기간·기법·자금·동시보유 수와 청산 방식을 바꿔 비교합니다.</p></div></div><div id="btLabBody"><div class="paper-empty">백테스트 데이터를 불러오는 중…</div></div></article>';
  ...
  function enhance(){
    ...
    if(form&&!root.querySelector('#btLabCapacity')){const l=document.createElement('label');l.textContent='최대 동시보유';l.innerHTML+=' <select id="btLabCapacity"><option>1</option><option>2</option><option selected>3</option><option>5</option><option>10</option></select>';form.appendChild(l)}
    ...
    document.querySelectorAll('.bt-packs').forEach(x=>x.remove());const legacySma=document.querySelector('.bt-extra-box [data-bt-strategy="sma200_20_squeeze"]');legacySma?.closest('.bt-extra-box')?.remove();
    if(window.SwingSequenceReplay&&!window.__capacityPatch){window.__capacityPatch=true;const base=window.SwingSequenceReplay.run;window.SwingSequenceReplay.run=(rows,capital,opt={})=>base(rows,capital,{capacity:Number(document.getElementById('btLabCapacity')?.value||3),...opt})}
    if(!document.getElementById('btLabExtraStyle')){const s=document.createElement('style');s.id='btLabExtraStyle';s.textContent='.bt-fast-period{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin:10px 0}.bt-fast-period b{font-size:9px;color:#7b837e}.bt-fast-period button,#btLabCapacity{border:1px solid #dfe4e0;background:#fff;border-radius:999px;padding:7px 10px;font-size:9px}.bt-fast-period button.on{background:#1c211e;color:#fff}.btlab-result-empty{padding:24px;border:1px dashed #dfe4e0;border-radius:16px;text-align:center;color:#8b938e}.btlab #btLabResult{white-space:normal!important}.btlab-card{box-shadow:0 8px 24px rgba(25,31,27,.035)}#btLabStrategies{display:flex;gap:8px 12px;flex-wrap:wrap;margin:12px 0}#btLabStrategies label{font-size:10px}#btLabStrategies label:has(input:disabled){opacity:.38}';document.head.appendChild(s)}
  }
