(()=>{
  ...
  function injectStyles(){
    ...
    const style=document.createElement('style');style.id='paperUiStyles';style.textContent=`
      ...
      .paper-head-note{font-size:11px;color:#8d959b;line-height:1.55;max-width:600px}.paper-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin:18px 0}.paper-stat{padding:15px;border:1px solid #e8ebee;border-radius:16px;background:#fff}.paper-stat span{display:block;color:#929aa1;font-size:10px;margin-bottom:5px}.paper-stat b{font-size:17px;letter-spacing:-.04em}.paper-actions{display:flex;gap:8px;flex-wrap:wrap}.paper-orders{display:grid;gap:10px}.paper-order{padding:16px;border:1px solid #e8ebee;border-radius:17px;background:#fff}.paper-order-top{display:flex;justify-content:space-between;gap:12px}.paper-order-name{font-weight:850;font-size:14px}.paper-order-sub{font-size:10px;color:#929aa1;margin-top:3px}.paper-origin{display:inline-flex;margin-left:5px;padding:2px 6px;border-radius:999px;background:#eceeea;color:#676d66;font-size:8px;font-weight:900;vertical-align:1px}.paper-origin.official{background:#151714;color:#fff}.paper-status{font-size:10px;font-weight:900;border-radius:999px;padding:5px 8px;height:max-content;background:#f0f2f3;color:#727a82}.paper-status.filled{background:#eaf6f0;color:#1d7c58}.paper-status.closed{background:#eef2fa;color:#4c66a2}.paper-status.cancelled{background:#f4f4f5;color:#8a8f94}.paper-order-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-top:13px}.paper-order-grid span{display:block;font-size:9px;color:#9ba1a7;margin-bottom:3px}.paper-order-grid b{font-size:11px}.paper-pnl.up{color:#b33c3c}.paper-pnl.down{color:#3569b5}.paper-empty{padding:24px;border:1px dashed #dfe4e2;border-radius:16px;color:#8b9398;font-size:12px;text-align:center}
  ...
  function originLabel(o){return o?.order_origin==='CONFIRMED_CLOSE'?'마감 확정':o?.order_origin==='LIVE_CANDIDATE'?'장중 실험':'구버전'}
  function originClass(o){return o?.order_origin==='CONFIRMED_CLOSE'?'official':''}
  function orderHTML(o){
    ...
    return `<div class="paper-order"><div class="paper-order-top"><div><div class="paper-order-name">${esc(o.name_ko||o.symbol)} <span class="ticker">${esc(o.symbol)}</span></div><div class="paper-order-sub">${esc(o.strategy_name||o.strategy_id)} · ${esc(o.signal_date||o.submitted_market_date||'')}<span class="paper-origin ${originClass(o)}">${originLabel(o)}</span></div></div><span class="paper-status ${statusClass(o.status)}">${esc(statusLabel(o.status))}</span></div><div class="paper-order-grid"><div><span>수량</span><b>${Number(o.qty||0).toLocaleString()}주</b></div><div><span>${o.status==='PENDING'?'예정 진입':'체결가'}</span><b>${usd(entry)}</b></div><div><span>가상 투입금</span><b>${krw(cost)}</b></div><div><span>손익</span><b class="paper-pnl ${pnlClass}">${pnl==null?'—':`${pnl>=0?'+':''}${krw(pnl)}${ret!=null?` · ${ret>=0?'+':''}${ret.toFixed(2)}%`:''}`}</b></div><div><span>TARGET</span><b>${usd(o.target)}</b></div><div><span>STOP</span><b>${usd(o.stop)}</b></div><div><span>보유 일봉</span><b>${Number(o.held_bars||0)} / ${Number(o.max_hold_bars||0)}</b></div><div><span>종료 사유</span><b>${esc(o.exit_reason||o.cancel_reason||'—')}</b></div></div></div>`;
  }
  function renderPaper(data){
    paperSnapshot=data;const sum=data?.summary||{},all=data?.orders||[];const summary=document.getElementById('paperSummary'),orders=document.getElementById('paperOrders'),meta=document.getElementById('paperMeta');
    ...
    if(orders){const rows=[...all].reverse();orders.innerHTML=rows.length?rows.map(orderHTML).join(''):'<div class="paper-empty">아직 가상 주문이 없어요. 실시간 후보 상세에서 ‘장중 가상매수’를 누르면 연구용 다음 시가 대기 주문이 만들어집니다.</div>'}
    if(meta){const live=all.filter(o=>o.order_origin==='LIVE_CANDIDATE').length,official=all.filter(o=>o.order_origin==='CONFIRMED_CLOSE').length;meta.textContent=`대기 ${sum.pending_orders||0} · 보유 ${sum.open_positions||0} · 종료 ${sum.closed_trades||0} · 장중실험 ${live} · 마감확정 ${official} · 실제주문 OFF`}
  }
  ...
  function setupDetailEnhancements(detail){
    ...
    const actions=detail.querySelector('.detail-top .actions');let buy=document.getElementById('paperBuy');if(actions&&!buy){buy=document.createElement('button');buy.id='paperBuy';buy.type='button';buy.className='paper-buy';buy.textContent='장중 가상매수';buy.disabled=true;actions.insertBefore(buy,actions.firstChild)}
    ...
    if(buy)buy.onclick=async()=>{
      ...
      buy.disabled=true;buy.textContent='가상주문 생성 중…';msg.className='paper-detail-message show';msg.textContent='장중 최신 스캔의 BUY/TARGET/STOP으로 연구용 다음 거래일 시가 대기 주문을 만들고 있어요.';
      try{const data=await apiJSON('/api/paper/submit',{method:'POST',body:JSON.stringify({symbol:d.symbol,strategy:d.strategy_id,source:'live_candidate'})});renderPaper(data);msg.className='paper-detail-message show';msg.textContent='장중 실험 가상주문이 생성됐어요. 공식 마감추천 성과와는 별도로 봅니다.';buy.textContent='장중 주문 생성됨'}catch(e){msg.className='paper-detail-message show err';msg.textContent=e.message;buy.textContent='장중 가상매수';buy.disabled=false}
    };
    const base=typeof window.openDetail==='function'?window.openDetail:null;
    if(base&&!window.__paperWrappedOpenDetail){window.__paperWrappedOpenDetail=true;window.openDetail=async function(symbol,strategy){const result=await base(symbol,strategy);let d=null;try{if(typeof detailData!=='undefined')d=detailData}catch{}renderDetailExplanation(d);const b=document.getElementById('paperBuy'),m=document.getElementById('paperDetailMessage');if(m)m.className='paper-detail-message';if(b){const current=!!d&&d.source!=='saved_history';b.disabled=!current;b.textContent=current?'장중 가상매수':'마감 확정 주문은 별도 모드 예정';b.title=current?'현재 장중 후보를 연구용 Paper 주문으로 생성합니다. 공식 마감추천 성과와 분리됩니다.':'과거 확정 기록을 현재 장중 실험 주문으로 재사용하지 않습니다.'}return result}}
  }
